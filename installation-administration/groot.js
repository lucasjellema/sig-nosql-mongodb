...
if( "groot" != db ) { 
    print("want 'db' to be 'groot' when the shell starts. terminating.");
    throw "use groot db before running script";
}

homework.init = function() { 
    var t = db.veel;
    if( t.count() ) { 
        throw "groot.veel not empty, so won't init(). If you want to reinit, drop the collection first.";
    }
    o = { 
        ticker : 'abcd',
...
homework.a = function() { assert( db == "groot" ); if( db.isMaster().msg != "isdbgrid" ) { throw "connect to a mongos not a mongod. try again."; } return db.veel.count();
}
...
homework.b = function() { 
    ...
    var x = db.getSisterDB("config").chunks.find({ns:"groot.veel"}).count();
    return x <= 2 ? x : 3;
}
homework.c = function() { 
    var x = db.getSisterDB("config").chunks.aggregate( [  { $match : { ns : "groot.veel" } } ,   { $group : { _id : "$shard" , n : { $sum : 1 } } } ] );
